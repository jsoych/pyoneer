    ...
    switch (value1->type) {
        ...
        case json_array:
            ...
            for (unsigned int i = 0; i < value1->u.array.length; i++) {
                ...
                if (r != 0) return json_array;
            }
            return 0;

        case json_object:
            if (value1->u.object.length != value2->u.object.length) return json_object;
